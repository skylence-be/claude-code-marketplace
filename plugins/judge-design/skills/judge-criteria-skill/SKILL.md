...
Once you know which action boundary needs a judge, design what the judge evaluates and what the actor agent must submit before any action executes. Output is the spec the judge prompt is built from.

## When to Use

**Good fit:** after action-surface-audit-skill has identified a boundary, before writing the judge prompt itself.

**Bad fit:** before the boundary is identified (run action-surface-audit-skill first), or when the criteria are obvious enough to write the judge prompt directly.

## What You Need

- The specific action boundary (e.g., outbound email, PR merge, CRM update)
- The domain (sales, engineering, support, finance, ops)
...
**Step 1.** Ask the user about the boundary, domain, policy, exposure, and failure history. Follow up if authorization is unclear, sensitivity is unclear, reversibility is unclear, or written policy is missing.

**Step 2.** Produce two deliverables:

### A. Judge Criteria Specification

Organized into four categories. Each criterion is a testable question the judge can answer yes/no or with a confidence level.

- **Authorization:** What constitutes valid authorization? How might the actor extend or misinterpret authorization? Difference between "user asked" and "user implied".
- **Evidence:** What sources of truth must the actor cite? What makes evidence sufficient vs insufficient? What staleness, ambiguity, or contradiction should the judge flag?
...
The structured object the actor submits before execution. The judge inspects it. Standard fields:

- Intended action (what specifically will happen)
- Reason (why this is the right next step)
- Supporting evidence (with sources)
- Authorization basis (where the user authorized this, with quotes/refs)
- Expected consequence (what changes in the world)
...
Customize fields to the action type — an email proposal differs from a code merge proposal.

**Step 3.** End with the three most common failure modes at this boundary and how the criteria are designed to catch each.
...
- **Action Proposal Format:** structured template with labeled fields, what each must contain, what makes it insufficient, and an example of a well-formed proposal.
...
- Do not invent policies the user hasn't described. If policy gaps exist, flag them and suggest what to define. Make criteria specific enough to test without becoming bureaucratic for low-risk cases.
- If authorization is ambiguous ("kind of implied"), design criteria that surface the ambiguity rather than resolving it silently.
