...
The first step of designing the judge layer for any agent: enumerate what it can do, classify each action by consequence severity, and decide which boundaries need judgment first.
...
Have these ready before invoking this skill — the audit is only as good as the inputs:

- What the agent does (workflows handled)
...
**Step 1.** Ask the user about the agent system. Get clear answers on workflows, tools, side effects, affected parties, and existing review processes. Ask follow-ups for edge cases: action chaining, agent-to-agent handoffs, agent-written memory used as future instructions.

**Step 2.** Once the action surface is clear, produce the audit:

1. List every distinct action the agent can take.
...
4. Produce a prioritized build plan ordered by consequence × frequency.

**Step 3.** End with a single-action recommendation: which boundary to instrument first.
...
- **First boundary recommendation:** the single action boundary to build the judge for first, with reasoning.

## Guardrails

- Classify only based on what the user describes. Do not invent capabilities.
- If side effects are vague, ask — do not assume safety.
- Flag ambiguous classifications and explain the uncertainty.
...
Once an action boundary is chosen, use **judge-criteria-skill** to design what the judge evaluates and **judge-prompt-writer-skill** to write the actual judge prompt. Use **judge-eval-suite-skill** to test the resulting judge.
