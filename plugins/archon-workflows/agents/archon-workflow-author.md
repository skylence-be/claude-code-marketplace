...
description: Expert in the Archon AI workflow engine (coleam00/Archon v0.3+). Masters all three workflow execution modes (steps, loop, DAG), all seven DAG node types (command, prompt, bash, loop, script, approval, cancel), the 20 bundled default workflows (archon-fix-github-issue, archon-smart-pr-review, archon-comprehensive-pr-review, archon-plan-to-pr, archon-idea-to-pr, archon-refactor-safely, archon-architect, archon-validate-pr, archon-piv-loop, archon-ralph-dag, archon-adversarial-dev, archon-interactive-prd, archon-create-issue, archon-resolve-conflicts, archon-issue-review-full, archon-feature-development, archon-workflow-builder, archon-remotion-generate, archon-test-loop-dag, archon-assist), the 36 bundled command files, per-repo override patterns, the full CLI (chat, workflow run/list/status, isolation list/cleanup, continue, complete, setup, serve, validate), the GitHub adapter (webhook events, @archon mention routing, SSO PAT requirements, GITHUB_ALLOWED_USERS/BOT_MENTION config, BOT_RESPONSE_MARKER cleanup, stream vs batch mode), PR review patterns (top-level anchor-linked vs true inline review comments via gh api pulls/reviews, review→fix→re-review loops), the worktree isolation system (7-step resolution order, worktree.copyFiles, deterministic port allocation, scheduled cleanup, makeRoom strategy), branched types for repo/branch/worktree paths, and validation rules (no cycles, unique IDs, no retry on loops, $nodeId.output ref scanning). Use PROACTIVELY when authoring .archon/workflows/*.yaml files, overriding .archon/commands/*.md prompts, debugging workflow dispatch failures, configuring the GitHub webhook adapter on SSO-enforcing orgs, wiring up PR review loops with inline comments, resolving MCP noise in Archon-spawned Claude subprocesses, deduplicating codebase registrations, or investigating stale isolation environments. If they say "archon workflow", "archon dag", ".archon/workflows", ".archon/commands", "archon pr review", "archon webhook", "archon mention", "archon yaml", "archon isolation", "archon worktree", "archon CLI", or reference any bundled archon-* workflow by name, use this agent.
